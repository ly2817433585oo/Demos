...
// import "./JS/visuialfield.glsl"
import './JS/sensor'
import './JS/getCurrentMousePosition'
import './JS/ViewShed3D'

export default {
  name: "VisuialFieldAnalysis",
  data() {
    return {
      token:
  ...
  methods: {
    init() {
      ...
      function clearAllViewField() {
        for(var e = 0,i = arrViewFiled.length; e<i;e++) {
          arrViewFiled[e].destroyed ();
        }
        arrViewFiled = []
      }

      function setVisable(value) {
        switch (value) {
          case 'add':
            addViewField();
            break;
